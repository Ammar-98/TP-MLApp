...
import { useFetchTicketStatusFilterData } from "@/hooks/useFetchTicketStatusFilterData";
...
export default function TicketStatusFilter({ className }: any) {
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useFetchTicketStatusFilterData();
  ...
  const addToSelected = (item: any) => {
    setSelectedTicketStatus((prev: any) => [...prev, item]);
  };

  const removeFromSelected = (item: any) => {
    setSelectedTicketStatus((prev: any) => prev.filter((i: any) => i !== item));
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="mr-1 font-[300]">Ticket Status</div>
        ...
        <div className="overflow-hidden absolute mt-1 w-[300px] flex-col rounded-2xl border border-slate-400 bg-white z-30">
          ...
          >
            {({ index, style }) => {
              ...
              const isSelected = selectedTicketStatus.includes(item);
