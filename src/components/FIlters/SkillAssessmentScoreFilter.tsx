...
import { useFetchSkillAssessmentScoreFilterData } from "@/hooks/useFetchSkillAssessmentScoreFilterData";

export default function SkillAssessmentScoreFilter({ className }: any) {
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useFetchSkillAssessmentScoreFilterData();
  ...
  const addToSelected = (item: any) => {
    setSelectedSkillAssessmentScore((prev: any) => [...prev, item]);
  };

  const removeFromSelected = (item: any) => {
    setSelectedSkillAssessmentScore((prev: any) =>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="mr-1 font-[300]">Skill Assessment Score</div>
        {selectedSkillAssessmentScore.length > 0 && (
          <div className="bg-[#69C920] text-white font-medium text-[12px] mx-1 rounded-full w-[20px] h-[20px] flex items-center justify-center">
            {selectedSkillAssessmentScore.length}
          </div>
        )}
        <ChevronDown
        ...
        <div className="overflow-hidden absolute mt-1 w-[300px] flex-col rounded-2xl border border-slate-400 bg-white z-30">
          ...
          >
            {({ index, style }) => {
              ...
              const isSelected = selectedSkillAssessmentScore.includes(item);
