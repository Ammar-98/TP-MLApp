...
import { useFetchRSFilterData } from "@/hooks/useFetchRSFilterData";

export default function RSFilter({ className }: any) {
  ...
  const { RSFilter, setRSFilter } = useContext(AppContext);
  const { data: fetchedData, isLoading, isSuccess } = useFetchRSFilterData();
  ...
  const addToSelected = (item: any) => {
    setRSFilter((prev: any) => [...prev, item]);
  };

  const removeFromSelected = (item: any) => {
    setRSFilter((prev: any) => prev.filter((i: any) => i !== item));
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="mr-1 font-[300]">RS</div>
        {RSFilter.length > 0 && (
        ...
        <div className="overflow-hidden absolute mt-1 w-[300px] flex-col rounded-2xl border border-slate-400 bg-white z-30">
          ...
          >
            {({ index, style }) => {
              ...
              const isSelected = RSFilter.includes(item);
