...
import { useFetchLanguageSpokenFilterData } from "@/hooks/useFetchLanguageSpokenFilterData";

export default function LanguagesSpokenFilter({ className }: any) {
  ...
  const { selectedLanguageSpoken, setSelectedLanguageSpoken } =
    useContext(AppContext);
  const {
    data: fetchedData,
    isLoading,
    isSuccess,
  } = useFetchLanguageSpokenFilterData();
  ...
  const addToSelected = (item: any) => {
    setSelectedLanguageSpoken((prev: any) => [...prev, item]);
  };

  const removeFromSelected = (item: any) => {
    setSelectedLanguageSpoken((prev: any) =>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="mr-1 font-[300]">Languages Spoken</div>
        ...
        <div className="overflow-hidden absolute mt-1 w-[300px] flex-col rounded-2xl border border-slate-400 bg-white z-30">
          ...
          >
            {({ index, style }) => {
              ...
              const isSelected = selectedLanguageSpoken.includes(item);
