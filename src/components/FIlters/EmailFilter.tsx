...
import { useFetchEmailFilterData } from "@/hooks/useFetchEmailFilterData";
import AppContext from "@/context/AppContext";
import { Skeleton } from "../ui/skeleton";
import { FixedSizeList as List } from "react-window";

export default function EmailFilter({ className }: any) {
  ...
  const { selectedEmail, setSelectedEmail } = useContext(AppContext);
  const { data: fetchedData, isLoading, isSuccess } = useFetchEmailFilterData();
  ...
  const addToSelected = (item: any) => {
    setSelectedEmail((prev: any) => [...prev, item]);
  };

  const removeFromSelected = (item: any) => {
    setSelectedEmail((prev: any) => prev.filter((i: any) => i !== item));
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="mr-1 font-[300]">Email</div>
        ...
        <div className="overflow-hidden absolute mt-1 w-[300px] flex-col rounded-2xl border border-slate-400 bg-white z-30">
          ...
          >
            {({ index, style }) => {
              ...
              const isSelected = selectedEmail.includes(item);
