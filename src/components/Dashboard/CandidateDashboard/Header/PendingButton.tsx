...
import { pendingCandidateFormSchema } from "@/lib/validations/auth/authValidations";
...
export function PendingButton({ refetch }: ChildProps) {
  ...
  const form = useForm<z.infer<typeof pendingCandidateFormSchema>>({
    resolver: zodResolver(pendingCandidateFormSchema),
  });

  async function onSubmit(values: z.infer<typeof pendingCandidateFormSchema>) {
    console.log(values);
    ...
    if (candidate_Type === "CS") {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.success("Candidate Pending");
    ...
    <Dialog open={open} onOpenChange={setopen}>
      <DialogTrigger asChild>
        <button
          ...
          // }
          className={
            "bg-[white] text-black border-2 border-[#FFD236] text-sm  flex items-center gap-x-2 rounded-2xl py-1 px-6 shadow-sm "
          }
