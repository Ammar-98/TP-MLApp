...
export function InviteToHireFlixButton({ refetch }: ChildProps) {
    ...
    if (candidate_Type === "CS") {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.success("Candidate invited to Hireflix");
    ...
    <Dialog open={open} onOpenChange={setopen}>
      <DialogTrigger asChild>
        <button
          ...
          // }
          className={
            "bg-[white] border-2 border-black text-black text-sm  flex items-center gap-x-2 rounded-2xl py-1 px-6 shadow-sm "
          }
