...
const getData = async () => {
  const data_to_send: any = {
    ...
    datasets: [
      {
        label: "Applicant Failed",
        data: [30, 40, 35, 25, 30, 40, 45, 50],
        backgroundColor: "#86FE96",
        barPercentage: 0.5,
        barThickness: 6,
        maxBarThickness: 8,
...
const DailyScreeningCountPerRS = () => {
  const { data, isLoading, isRefetching }: any = useQuery({
    queryKey: ["DailyScreeningCountPerRS"],
    queryFn: () => getData(),
  });
  ...
  return (
    <div className="flex flex-col gap-y-4 relative w-1/2">
      <div className="flex items-center align-middle justify-between">
        <span className="font-semibold text-xl text-main-text pl-3">
          Daily Screening Count per RS
        </span>
      </div>
...
export default DailyScreeningCountPerRS;
